  ...
  table(dv, that) {
    ...
    const getTableContents = () => {
      ...
      for (let i = 0; i < sectionPaths.length; i++) {
        ...
        let content = "";
        let page = that.app.vault.getAbstractFileByPath(sectionPaths.values[i]);
      ...
      if (includeBibliographyEstimate) {
        ...
        if (!charactersIncludeSpaces) characterCount = citationCount 
        cumulativeShare += calculateShare(characterCount, wordCount);
      ...
      output.push([
        "Total",
        insert1000sep(totalChars),
        insert1000sep(totalWords),
        toPercentStr(totalProgress),
        naChar
      ]);
